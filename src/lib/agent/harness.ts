...
function getSystemPrompt(taskType: TaskType): string {
  const prompts: Record<TaskType, string> = {
    ...
    dream: `You are a memory synthesis agent. Review past CxO session logs and extract durable learnings, patterns, and strategic insights. Output valid JSON with fields: key_learnings (string array), patterns (string), strategic_insights (string).`,
  }
